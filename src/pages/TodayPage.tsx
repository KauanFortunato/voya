      ...
      <main className="today-page" id="main-content">
        ...
        >
          ...
          {highlightedActivity ? <section className="next-activity" aria-labelledby="next-activity-title">
            ...
            <div className="next-activity__actions"><a className="primary-action" href={highlightedActivity.mapsUrl} target="_blank" rel="noreferrer"><Map size={17} />Maps</a><button className="glass-action" type="button" onClick={() => setSelectedActivity(highlightedActivity)}>Detalhes</button><button className="complete-action" type="button" disabled={savingIds.includes(highlightedActivity.id)} aria-busy={savingIds.includes(highlightedActivity.id)} onClick={() => void toggleActivity(highlightedActivity)}><Check size={17} />{savingIds.includes(highlightedActivity.id) ? 'A guardar…' : 'Concluir'}</button></div>
